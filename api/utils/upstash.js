...
const STARTUP_PING_KEY = "aris:health:startup_ping";

/**
 * Run once when the API process starts. Warns if Redis is missing or unreachable so
 * operators know free-tier quota enforcement (and other Redis-backed features) may be down.
 */
export async function logRedisStartupHealth() {
  ...
  if (!hasUrl || !hasToken) {
    logger.warn("redis_quota_enforcement_disabled", {
      reason: "missing_env",
      has_upstash_redis_rest_url: hasUrl,
      has_upstash_redis_rest_token: hasToken,
      hint: "Set UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN in production (e.g. Vercel env for serverless API, or Railway). Without both, free-tier monthly audit quotas are not enforced server-side.",
    });
    return;
  }
  ...
  if (!redis) {
    logger.warn("redis_quota_enforcement_disabled", {
      reason: "client_uninitialized",
      hint: "Env vars were present but the Redis client was not created.",
    });
    return;
  }

  try {
    await redis.set(STARTUP_PING_KEY, "ok", { ex: 30 });
    const v = await redis.get(STARTUP_PING_KEY);
    if (v !== "ok") {
      logger.warn("redis_startup_check_failed", {
        reason: "unexpected_read",
        value: v == null ? null : String(v),
        hint: "Upstash responded but read-after-write did not match. Quota enforcement may be unreliable.",
      });
      return;
    }
    logger.info("redis_startup_ok", {
      quota_enforcement: "active",
      ping_key: STARTUP_PING_KEY,
    });
  } catch (err) {
    logger.warn("redis_startup_unreachable", {
      error: err?.message || String(err),
      hint: "Redis is unreachable — free-tier audit quota (3/month) falls back to permissive mode. Check Upstash dashboard, REST URL/token, and outbound network from this host.",
    });
  }
}
